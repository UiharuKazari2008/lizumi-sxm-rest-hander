...
function searchEvents() {
    ...
    Object.values(config.autosearch_terms).map(f => {
        events.filter(e => channelTimes.completed.indexOf(e.guid) === -1 && e.filename && e.filename.toLowerCase().includes(f.search.toLowerCase()) && e.duration > 900 && (!f.duration || (f.duration && e.duration > f.duration))).map(e => {
