...
async function registerBounce(addTime, channelNumber, format) {
...
async function bounceEventFile(eventsToParse, types) {
    ...
    const digitalRecFiles = fs.readdirSync(config.record_dir).filter(e => e.startsWith(config.digital_record_prefix) && e.endsWith(".mp3")).map(e => {
        return {
            date: moment(e.replace(config.digital_record_prefix, '').split('.')[0] + '', "YYYYMMDD-HHmmss"),
            file: e
        }
    });
    const analogRecTimes = analogRecFiles.map(e => e.date.valueOf());
    const digitalRecTimes = digitalRecFiles.map(e => e.date.valueOf());
            ...
            if ((eventItem.format && eventItem.format === "digital") || (types && types === "digital")) {
                try {
                    ...
                    if (digitalStartFile < 0)
                        digitalStartFile = 0
                    let digitalEndFile = findClosest(digitalRecTimes, eventItem.syncEnd)
                    if (digitalEndFile + 1 > digitalRecTimes.length)
                        digitalEndFile = digitalRecTimes.length - 1
                    const digitalFileItems = (digitalStartFile < digitalEndFile) ? digitalRecFiles.slice(digitalStartFile, digitalEndFile + 1) : [digitalRecFiles[digitalStartFile]]
                    ...
                    console.error(`ALERT: FAULT - Digital Extraction Failed: ${e.message}`)
                    console.error(e);
                }
            }
    ...
    if (req.params.display) {
        switch (req.params.display) {
            ...
            case 'select_bounce_song':
                ...
                break;
            case 'modify_meta':
                modifyMetadataGUI(undefined, (req.query.ch) ? req.query.ch : undefined);
                res.status(200).send('OK')
                break;
            default:
