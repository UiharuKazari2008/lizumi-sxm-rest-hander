            ...
            switch (req.params.type) {
                ...
                case 'devices':
                    ...
                    const tuners = listTuners().map(e => {
                        ...
                        const channelMeta = (activeJob.length > 0) ? activeJob.map(j => getEvent(undefined, j.guid))[0] : (e.digital && watchdog_tuners[e.id].player_guid) ? getEvent(undefined, watchdog_tuners[e.id].player_guid) : (meta) ? meta : false
                        return {
                            id: e.id,
                            name: e.name,
                            channel: (() => {
